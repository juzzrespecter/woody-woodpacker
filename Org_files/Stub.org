...
#+begin_src c :tangle payload/elf/x64/all_in.c :mkdirp yes
  #include <elf.h>
  #include <sys/mman.h>
  #include <fcntl.h>
  #include <unistd.h>
...
#+begin_src asm :tangle payload/elf/x64/stage1.s :mkdirp yes
  global _start
...
#+begin_src asm :tangle payload/elf/x64/payload.s :mkdirp yes
  	global payload
    payload:
...
#+begin_src asm :tangle payload/elf/x64/stage2.s :mkdirp yes
  global _start
...
#+begin_src asm :tangle payload/elf/x64/stage3.s :mkdirp yes
global _start
...
#+begin_src c :tangle payload/elf/x32/all_in.c :mkdirp yes
  #include <elf.h>
  #include <sys/mman.h>
  #include <fcntl.h>
  #include <unistd.h>
...
#+begin_src asm :tangle payload/elf/x32/stage1.s :mkdirp yes
  global _start
...
#+begin_src asm :tangle payload/elf/x32/payload.s :mkdirp yes
  payload:
          call .woody_write	;;
  	times 14 ret
          .woody_write:
...
#+begin_src asm :tangle payload/elf/x32/stage2.s :mkdirp yes
   global _start:
        ;; Cheatsheet mmap
        ;; ebx addr
        ;; ecx size
...
#+begin_src asm :tangle payload/elf/x32/stage3.s :mkdirp yes
  	int 0x80
  	pop edx
  	ret
#+end_src
...
#+end_src
:END:
~xxd -p -c 0  bytecode/elf/x32/stage1.bin | sed "s/\(c3\)\{15\}/$a/g" | xxd -r -p > output_bin~
