...
#+begin_src c :tangle include/common.h :mkdirp yes
  ...
  //file validation
  int		get_file(int *fd, const char *path); //open
...
#+begin_src c :tangle srcs/cmn/stages.c :mkdirp yes
  ...
  t_buffer	load_stage_buffer(t_arch arch, t_bin_type bin_type)
    {
  	  static const char *const	binary_paths[NUM_ARCHS][NUM_BIN_TYPES] = {
        [ARCH_X86] = {
        ...
        stage_fd = open(binary_paths[arch][bin_type], O_RDONLY);
...
#+begin_src c :tangle src/elf/file.c :mkdirp yes
  #include "common.h"
