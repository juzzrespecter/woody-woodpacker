...
#+begin_src asm :tangle payload/x32/stage1.s :mkdirp yes
  ...
  .call_decrypt:
        	...
        	test eax, eax
        	jz _exit
        	pop edx		; (va) s2 ; [esp] (len, len, va) s2
        	push edx	;[esp] (va, len, len, va) s2.
        	push 0xfeffffff		; ;[esp] (len) c_s2, (va) s2,  (len, len, va) s2.
        	push eax 	;[esp] (va, len) c_s2, (va, len, len, va) s2.
        	push edx	;[esp] ((dst) s2, (va, len) c_s2) func params, (va, len, len, va) s2.
        	call lz77_decompress	;Previous 3 B -> New 8 B
  	test eax, eax
        	jnz _exit		;Fine
  .munmap_decrypt: 	;[esp] ((dst) s2, (va, len) c_s2) func params, (va, len, len, va) s2.
  	pop ebx			;(va) s2,[esp] (va, len) c_s2) func params, (va, len, len, va) s2.
  	pop ebx			;(va) c_s2[esp] ((len) c_s2) func params, (va, len, len, va) s2.
          pop ecx 		;(len) c_s2[esp] (va, len, len, va) s2.
          push 91			;munmap[esp] (va, len, len, va) s2.
  	pop eax			;[esp] (va, len, len, va) s2.
  	int 0x80		;[esp] (va, len, len, va) s2.
  .set_var_stage_2:		;[esp] (va, len, len, va) s2.
  	pop ebx			;(va) s2 [esp] (len, len, va) s2.
        	pop edx			;(len) s2 [esp] (len, va) s2
  .mprotect_stage_2:
        	push 0x5		;Flags R-X
