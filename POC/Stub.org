...
* How to Improve:
- [-] Reduce bytecode.
  - [X] rep, movsb, stosb, enter, leave
  - [ ] Erase function just used once. (set_auxv, get_at interp, write_dyn, ...)
  - [ ] Refactor the code for don't need to got the setup for syscalls (mmap/2, mproctect) more than once.
  - [ ] Try use loop beside test/cmp jcc .loop_start_pos.
