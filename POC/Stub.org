...
#+begin_src asm :tangle payload/x64/stage1.s :mkdirp yes
  ...
  .return_crypt_compress_addr:
  	pop rsi		;src;;[rsp](len) s2, (fd, len, vaddr) s1
  	pop rdx
  	push rdx
  	sub rsp, rdx
  	push rsp		;dest
  	pop rdi
  	push rdx	;[rsp] (len|buffer_len) s2|decrypt, (buffer) decrypt, (len) s2, (fd, len, vaddr) s1
  ...
  .call_decrypt:
  	...
  	pop rdx			;(len) s2|buffer [rsp] (buffer)  decrypt, (len) s2, (fd, len, vaddr) s1
  	add rsp, rdx ;Restore stack [rsp](len) s2, (fd, len, vaddr) s1
  	test rax, rax
  	jnz _exit
  ...
  .mprotect_stage_2:
  	...
  	push 0x5
  	pop rdx			;prot R-X
  	pop rsi			;(len) s2 [rsp] (fd, len, vaddr) s1
  	push r13		;[rsp] (va) s2,(fd, len, vaddr) s1
  	push rsi		;[rsp] (len, va) s2, (fd, len, vaddr) s1
  	push 0xA
