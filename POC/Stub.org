...
#+end_src
:END:
#Try to change the ~pop ebx, sub esp, 10~ -> ~sub esp, 10pop ebx~
#+begin_src asm :tangle payload/x32/stage1.s :mkdirp yes
  ...
  .return_crypt_compress_addr:
  	...
  	pop edx
  	push edx
  				;Comment the line below. - 2 B to the insertion affect aes key....
        	sub esp, edx
  	;;       	mov edi, esp	 ;edi = buffer out; edx = len out;esi -> in; edx in_len
        	push edx	;[esp] (len, buffer) decrypt, (len, va) s2
        	push eax	;[esp] (va) s2, (len, buffer) decrypt, (len, va) s2
  ...
  .call_decrypt:
        	...
        	test eax, eax
        	jz _exit
        	pop edx		; (va) s2 ; [esp] (len, buffer) decrypt, (len, va) s2
        	push 0		;[esp] (padding),  (len, buffer) decrypt, (len, va) s2.
        	push edx	;[esp] (va) s2, padding,(len,buffer) decrypt , (len, va) s2.

        	push 0xfeffffff		; len
        	push eax 	;[esp] (va, len) c_s2, (va) s2, padding,(len,buffer) decrypt , (len, va) s2.
        	push edx	;[esp] ((dst) s2, (va, len) c_s2) func params, (va) s2, padding,(len,buffer) decrypt , (len, va) s2.
        	call lz77_decompress
        	pop ebx	; (va)s2 [esp](len) c_s2, (va) s2, padding,(len,buffer) decrypt , (len, va) s2.
        	add esp, 0x10		;[esp]
        	pop edx			;(len) decrypt [esp] (buffer) decrypt, (len, va) s2
  	;;  ;comment the line below -2 B to the insertion affect expected va ...
        	add esp, edx ;Restore stack [esp] (len, va) s2
        	test eax, eax
        	jnz _exit		;Fine
...
Section Headers:
  ...
  #+end_quote
:END:
using off for skip and len for count.
~dd if=path.o of=dst skip=$((0xoff)) bs=1 count=$((0xlen))~
add string in case of stage1
:fold:
#+begin_src sh
hexdump -h
Options:
