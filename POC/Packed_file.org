...
#+begin_src c :tangle srcs/elf/packer/elf_packer.c :mkdirp yes
  							  ...
  							  t_stage1_params *params)
  {
  	...
  	const size_t stage1_patch_offsets[NUM_ARCHS][STAGE1_PATCH_COUNT] = {
  		[ARCH_X86] = { compute_patch_offset(ARCH_X86, 0xa1, g_stage1, patchs_delta_count),
  					   compute_patch_offset(ARCH_X86, 0xb3, g_stage1, patchs_delta_count),
  					   compute_patch_offset(ARCH_X86, 0x0c, g_stage1, patchs_delta_count),
  					   compute_patch_offset(ARCH_X86, 0x65, g_stage1, patchs_delta_count),
  					   compute_patch_offset(ARCH_X86, 0xc6, g_stage1, patchs_delta_count),
  					   compute_patch_offset(ARCH_X86, 0x3f, g_stage1, patchs_delta_count)},
  		[ARCH_X64] = { compute_patch_offset(ARCH_X64, 0x08, g_stage1, patchs_delta_count),
  					   compute_patch_offset(ARCH_X64, 0x1f, g_stage1, patchs_delta_count),
  					   compute_patch_offset(ARCH_X64, 0x60, g_stage1, patchs_delta_count),
  					   compute_patch_offset(ARCH_X64, 0xc0, g_stage1, patchs_delta_count),
  					   compute_patch_offset(ARCH_X64, 0xf0, g_stage1, patchs_delta_count),
  					   compute_patch_offset(ARCH_X64, 0x98, g_stage1, patchs_delta_count)}
  	};
  	...
  	for (size_t i = 0; i < STAGE1_PATCH_COUNT; i++)
  	{
  		printf("s1_off[ARCH_X86][%lu] = %4lx = %8lu\n", i, stage1_patch_offsets[ARCH_X86][i], stage1_patch_offsets[ARCH_X86][i] );
  		printf("s1_off[ARCH_X64][%lu] = %4lx = %8lu\n", i, stage1_patch_offsets[ARCH_X64][i], stage1_patch_offsets[ARCH_X64][i]);
  	}
      ...
      if (invalid_buffer(&stage1))
  	{
    	...
    	size_t expected_va = elf_get_highest_loadable_vaddr_avalaible(
    		ehdr, class, ctx.phnum) + ctx.prefix_size + (size_t)ft_ternary(
    			ctx.arch, (void *)(long int)0x5, (void*)(long int)
    			(compute_patch_offset(ctx.arch, 0x8e, g_stage1,
    								  sizeof(g_stage1)/sizeof(*g_stage1))));

    	size_t s1_len = stage1.len + ctx.prefix_size + params->encrypted.len;

    	t_patch_desc patches_desc[] = {
            (t_patch_desc){ .base_off_x86 = 0xa1, .base_off_x64 = 0x08,
    		  .len_x64 = sizeof(uint64_t), .len_x86 = sizeof(uint32_t),
              .value = &expected_va },
            (t_patch_desc){ .base_off_x86 = 0xb3, .base_off_x64 = 0x1f,
    		  .len_x64 = sizeof(uint64_t), .len_x86 = sizeof(uint32_t),
              .value = &s1_len },
            (t_patch_desc){ .base_off_x86 = 0x0c, .base_off_x64 = 0x60,
    		  .len_x64 = sizeof(uint64_t), .len_x86 = sizeof(uint32_t),
              .value = &params->stage2_total_size },
            (t_patch_desc){ .base_off_x86 = 0x65, .base_off_x64 = 0xc0,
    		  .len_x64 = sizeof(uint64_t), .len_x86 = sizeof(uint32_t),
              .value = &params->compressed_hdr_size },
            (t_patch_desc){ .base_off_x86 = 0xc6, .base_off_x64 = 0xf0,
    		  .len_x64 = sizeof(uint64_t), .len_x86 = sizeof(uint32_t),
              .value = (void *)&ctx.s2_entry_offset },
            // AES key patch
            (t_patch_desc){ .base_off_x86 = 0x3f, .base_off_x64 = 0x98,
    		  .len_x64 = sizeof(params->key), .len_x86 = sizeof(params->key),
              .value = &params->key.ints[0] }
        };
    	if (sizeof(patches_desc))
    		ft_printf("patches_desc_exists\n");
  	...
  	/* apply_patches_to_buffer_with_patch_desc(&stage1, ctx.arch, patches_desc, sizeof(patches_desc) / sizeof (*patches_desc), g_stage1, sizeof(g_stage1) / sizeof (*g_stage1)) */
      return (stage1);
  }
