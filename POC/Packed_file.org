...
#+begin_src c :tangle srcs/elf/packer/elf_packer.c :mkdirp yes
  ...
  static bool elf_set_s3_get_info(const t_elf_h *ehdr, unsigned char class, uint64_t phnum, size_t gap_size, uint64_t *ret_vaddr, uint64_t *ret_offset, t_elf_ph *phdr)
  {
      ...
      if (!ehdr || !phnum || ! gap_size)
  		return (false);
      ph_ndx = 0;
      while (ph_ndx < phnum)
  	{
  	    ,*phdr = elf_get_program_header(ehdr, ph_ndx++);
  	    if (!is_exec_load_phdr(phdr, class))
  		    continue;
  	    avalaible_vaddr = get_vaddr_gap_fits(phdr, class, gap_size);
  	    if (avalaible_vaddr == 0)
  		    continue;
  	    avalaible_offset = get_offset_gap_fits(ehdr, phdr, class, phnum, gap_size);
  									 ...
  									 t_stage1_params *params)
  {
  	...
  	if (invalid_buffer(&stage1))
  		return (stage1);
      if (invalid_aes(&params->key))
  		ft_memset(params->key.ints, s2_entry_offset,
  				  sizeof(params->key.ints)/sizeof(*params->key.ints));
      t_type stage1_patches[STAGE1_PATCHES] = {
  		...
  		{.b8 = (uint64_t)params->key.ints[0]},//Encryption key in case x86 use 32b
  		{.b8 = (uint64_t)params->key.ints[0]},//In x64 overwrite some off with eq val
  		{.b8 = (uint64_t)params->key.ints[0]},
  		{.b8 = (uint64_t)params->key.ints[0]}
      };
