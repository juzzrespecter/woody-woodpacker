...
#+begin_src c :tangle srcs/elf/packer/elf_packer.c :mkdirp yes
  						 ...
  						 const t_buffer *stage1)
  {
      patches[0].b8 =
          ...
          + ctx->prefix_size + 5 + (!ctx->arch * 0x72);
...
:END:
****** TODO del the pass of the param, Update offsets and values in C # Check x86 & x64 & valgrind.
******* DONE in interp
...
***** NEXT Move the open to the end of asm, Update offsets and values in C. # Check x86 & x64 & Valgrind.
***** HOLD When s1 is fine modify s2 # Check x86 & x64 & Valgrind.
**** x86 & x64
***** PLAN Set correctly the aes key.
