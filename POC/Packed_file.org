...
#+begin_src c :tangle srcs/elf/packer/elf_packer.c :mkdirp yes
  							  ...
  							  t_stage1_params *params)
  {
  	const size_t stage1_patch_offsets[NUM_ARCHS][STAGE1_PATCHES] = {
  		//Removable:              xxxxx,                       ,xxxx,xxxx,xxxx
  		//Val:  expected va,s1len,s2len,s2len,c_len,offs2,aesky,
  		//Index:          0,    1,    2,   3,    4,    5,    6, Unused
          [ARCH_X86] = { 0xa1, 0xb3, 0x0c, 0x0c, 0x65, 0xc6, 0x3f, 0x0, 0x0, 0x0 },
          [ARCH_X64] = { 0x08, 0x1f, 0x60, 0x60, 0xc0, 0xf0, 0x98, 0x0, 0x0, 0x0}
      };
      ...
      if (ctx.arch == ARCH_X64 || ctx.arch == ARCH_X86)
        ft_memcpy(stage1.data + patch.offsets[6], &params->key.ints[0], sizeof(params->key.ints));
      return (stage1);
  }
