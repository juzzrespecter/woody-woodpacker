...
#+begin_src shell
  ...
  stage=stage1|stage2|payload
   xxd -p bytecode/x32/stage1.bin | sed "s/\(c3\)\{15\}/$a/g" | xxd -r -p > output_bin
   xxd -p bytecode/x32/payload.bin | sed "s/\(c3\)\{14\}/$woody/g" | xxd -r -p > output_bin
...
#+begin_src c :tangle srcs/elf/packer/elf_packer.c :mkdirp yes
  						 ...
  						 const t_buffer *stage1)
  {
      patches[0].b8 =
          ...
          + ctx->prefix_size + 0x5 + (!ctx->arch * (0x72 + 0x1a - 3));
  							  ...
  							  t_stage1_params *params)
  {
  	const size_t stage1_patch_offsets[NUM_ARCHS][STAGE1_PATCHES] = {
          [ARCH_X86] = { 0x8a + 0x1a - 3, 0x9c + 0x1a - 3, 0x0c, 0xaf + 0x1a - 3, 0x4e + 0x1a - 3, 0xaf + 0x1a - 3, 0x3f, 0xaf - 3, 0xaf - 3, 0xaf - 3 },
          [ARCH_X64] = { 0x08, 0x1f, 0x60, 0x60, 0xa0 + 0x1a - 8, 0xce + 0x1a - 10, 0x8c, 0xce - 10, 0xce - 10, 0xce - 10 }
      };
...
#+end_src
***** Don't use stack
for huge data per example decrypt.
Better use stage1 map throught mprotect for allow read and write.
